...
struct FUEAgentToolParam
{
	...
	FUEAgentToolParam(FString InName, FString InType, FString InDesc, bool bInRequired = false, FString InDefault = FString(), FString InItemsType = FString())
...
struct FUEAgentToolSchema
{
	FString Name;
	FString Category;
	FString Description;
	TArray<FUEAgentToolParam> Params;

	FUEAgentToolSchema(FString InName, FString InCategory, FString InDesc, TArray<FUEAgentToolParam> InParams = {})
		: Name(MoveTemp(InName))
	...
	TSharedPtr<FJsonObject> ToJsonSchema() const
	{
		...
		for (const FUEAgentToolParam& Param : Params)
		{
