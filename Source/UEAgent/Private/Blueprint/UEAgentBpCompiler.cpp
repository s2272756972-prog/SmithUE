...
FUEAgentBpCompiler::FUEAgentBpCompiler(UBlueprint* InBlueprint, UEdGraph* InGraph)
	: Blueprint(InBlueprint), Graph(InGraph), NextNodeX(300), NextNodeY(0)
{
}

FUEAgentCompileResult FUEAgentBpCompiler::CompileFunction(UBlueprint* Blueprint, const FString& Code)
{
	FUEAgentCompileResult Result;
	...
	UEdGraph* Graph = FUEAgentBpAtomicAPI::FindGraph(Blueprint, Signature.FunctionName);
	{
		const FScopedTransaction Transaction(NSLOCTEXT("UEAgent", "CompileDslFunction", "UEAgent: Compile Blueprint DSL Function"));
		if (Graph)
		{
	...
	FUEAgentBpCompiler Compiler(Blueprint, Graph);
	...
	if (Result.Errors.Num() == 0)
	{
		...
		if (!FUEAgentBpAtomicAPI::CompileBlueprint(Blueprint, CompileErrors, /*bSkipGarbageCollection=*/true)) { Result.Errors.Append(CompileErrors); }
	}
	Result.bSuccess = Result.Errors.Num() == 0;
	return Result;
}

bool FUEAgentBpCompiler::ValidateSyntax(const FString& Code, FString& OutError)
{
	OutError.Reset();
	const TArray<FString> Lines = SplitLines(Code);
	const int32 SignatureLine = FindFirstNonEmptyLine(Lines);
...
bool FUEAgentBpCompiler::Parse(const FString& Code)
{
	const TArray<FString> Lines = SplitLines(Code);
	const int32 SignatureLine = FindFirstNonEmptyLine(Lines);
...
bool FUEAgentBpCompiler::ParseSignature(const FString& SignatureLine)
{
	FCompilerSignature Signature;
...
void FUEAgentBpCompiler::ProcessLines(const TArray<FString>& Lines, int32& LineIndex, const FString& LastExecNodeId, const FString& LastExecPinName)
{
...
void FUEAgentBpCompiler::ProcessFunctionCall(const FString& Line, FString& InOutNodeId, FString& InOutPinName)
{
	int32 OpenIndex = INDEX_NONE, CloseIndex = INDEX_NONE;
...
void FUEAgentBpCompiler::ProcessIfStatement(const TArray<FString>& Lines, int32& LineIndex, FString& InOutNodeId, FString& InOutPinName)
{
...
void FUEAgentBpCompiler::ProcessForLoop(const TArray<FString>& Lines, int32& LineIndex, FString& InOutNodeId, FString& InOutPinName)
{
	int32 OpenIndex = INDEX_NONE, CloseIndex = INDEX_NONE;
	const FString Header = Lines[LineIndex].TrimStartAndEnd();
...
void FUEAgentBpCompiler::ProcessAssignment(const FString& Line, FString& InOutNodeId, FString& InOutPinName)
{
	...
	const FString SetNodeId = FUEAgentBpAtomicAPI::CreateNode(Blueprint, Graph, TEXT("K2Node_VariableSet"), FVector2D(NextNodeX, NextNodeY), Params);
	...
	if (bLiteral) { if (!FUEAgentBpAtomicAPI::SetPinDefault(Blueprint, Graph, SetNodeId, ValuePin, A)) { AddError(FString::Printf(TEXT("Failed to set variable value for %s"), *VarName)); } }
	else if (!A.IsEmpty() && !B.IsEmpty()) { ConnectData(A, B, SetNodeId, ValuePin); }
	InOutNodeId = SetNodeId; InOutPinName = TEXT("then"); NextNodeX += 300;
}

void FUEAgentBpCompiler::ProcessReturn(const FString& Line, FString& InOutNodeId, FString& InOutPinName)
{
	...
	if (!Expr.IsEmpty())
	{
		...
		else if (bLiteral) { if (!FUEAgentBpAtomicAPI::SetPinDefault(Blueprint, Graph, ReturnNodeId, TEXT("ReturnValue"), A)) { AddError(TEXT("Failed to set return value")); } }
...
void FUEAgentBpCompiler::ProcessLocalVariable(const FString& Line, FString& InOutNodeId, FString& InOutPinName)
{
...
FString FUEAgentBpCompiler::CreateCallFunctionNode(const FString& FunctionNameText, const TArray<TPair<FString, FString>>& Args)
{
	TSharedPtr<FJsonObject> Params = MakeShared<FJsonObject>();
	Params->SetStringField(TEXT("function_name"), FunctionNameText);
	const FString NodeId = FUEAgentBpAtomicAPI::CreateNode(Blueprint, Graph, TEXT("K2Node_CallFunction"), FVector2D(NextNodeX, NextNodeY), Params);
	...
	for (int32 Index = 0; Index < Args.Num() && Index < InputPins.Num(); ++Index)
	{
		...
		if (bLiteral) { if (!FUEAgentBpAtomicAPI::SetPinDefault(Blueprint, Graph, NodeId, InputPins[Index], A)) { AddError(FString::Printf(TEXT("Failed to set argument pin %s"), *InputPins[Index])); } }
...
FString FUEAgentBpCompiler::CreateBranchNode(const FString& ConditionExpression)
{
	const FString NodeId = FUEAgentBpAtomicAPI::CreateNode(Blueprint, Graph, TEXT("K2Node_IfThenElse"), FVector2D(NextNodeX, NextNodeY), MakeShared<FJsonObject>());
	...
	if (bLiteral) { if (!FUEAgentBpAtomicAPI::SetPinDefault(Blueprint, Graph, NodeId, TEXT("Condition"), A)) { AddError(TEXT("Failed to set branch condition")); } }
	else { ConnectData(A, B, NodeId, TEXT("Condition")); }
	NextNodeX += 300;
	return NodeId;
}

FString FUEAgentBpCompiler::CreateForLoopNode(const FString& StartExpr, const FString& EndExpr)
{
	TSharedPtr<FJsonObject> Params = MakeShared<FJsonObject>();
	Params->SetStringField(TEXT("macro_path"), TEXT("/Engine/EditorBlueprintResources/StandardMacros.StandardMacros:ForLoop"));
	const FString NodeId = FUEAgentBpAtomicAPI::CreateNode(Blueprint, Graph, TEXT("K2Node_MacroInstance"), FVector2D(NextNodeX, NextNodeY), Params);
	...
	for (const TPair<FString, FString>& Pair : LoopPins)
	{
		...
		if (bLiteral) { if (!FUEAgentBpAtomicAPI::SetPinDefault(Blueprint, Graph, NodeId, Pair.Key, A)) { AddError(FString::Printf(TEXT("Failed to set %s"), *Pair.Key)); } }
...
FString FUEAgentBpCompiler::ParseExpression(const FString& Expression)
{
	...
	if (IsIdentifier(Trimmed))
	{
		...
		const FString GetterNodeId = FUEAgentBpAtomicAPI::CreateNode(Blueprint, Graph, TEXT("K2Node_VariableGet"), FVector2D(NextNodeX, NextNodeY), Params);
...
TPair<FString, FString> FUEAgentBpCompiler::ParseFunctionCallExpression(const FString& Expr)
{
	int32 OpenIndex = INDEX_NONE, CloseIndex = INDEX_NONE;
...
void FUEAgentBpCompiler::AddError(const FString& Error)
{
	if (!Error.IsEmpty()) { Errors.Add(Error); UE_LOG(LogUEAgent, Warning, TEXT("UEAgentBpCompiler: %s"), *Error); }
}

void FUEAgentBpCompiler::ConnectExec(const FString& SourceNodeId, const FString& SourcePin, const FString& TargetNodeId, const FString& TargetPin)
{
	if (SourceNodeId.IsEmpty() || TargetNodeId.IsEmpty()) { return; }
	if (!FUEAgentBpAtomicAPI::ConnectPins(Blueprint, Graph, SourceNodeId, SourcePin, TargetNodeId, TargetPin)) { AddError(FString::Printf(TEXT("Failed exec connection %s.%s -> %s.%s"), *SourceNodeId, *SourcePin, *TargetNodeId, *TargetPin)); }
}

void FUEAgentBpCompiler::ConnectData(const FString& SourceNodeId, const FString& SourcePin, const FString& TargetNodeId, const FString& TargetPin)
{
	if (SourceNodeId.IsEmpty() || TargetNodeId.IsEmpty() || SourcePin.IsEmpty() || TargetPin.IsEmpty()) { return; }
	if (!FUEAgentBpAtomicAPI::ConnectPins(Blueprint, Graph, SourceNodeId, SourcePin, TargetNodeId, TargetPin)) { AddError(FString::Printf(TEXT("Failed data connection %s.%s -> %s.%s"), *SourceNodeId, *SourcePin, *TargetNodeId, *TargetPin)); }
}
