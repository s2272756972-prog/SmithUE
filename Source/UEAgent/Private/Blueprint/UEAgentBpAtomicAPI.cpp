...
using namespace UEAgentBpAtomicAPIHelpers;

void FUEAgentBpAtomicAPI::RegisterTools(FUEAgentToolRegistry& Registry)
{
	Registry.Register(FUEAgentToolSchema(TEXT("bp_create"), TEXT("Blueprint"), TEXT("Create a new Blueprint asset"), { FUEAgentToolParam(TEXT("name"), TEXT("string"), TEXT("Blueprint asset name"), true), FUEAgentToolParam(TEXT("parent_class"), TEXT("string"), TEXT("Parent class name"), true), FUEAgentToolParam(TEXT("save_path"), TEXT("string"), TEXT("Destination content path"), true) }), &HandleBpCreate);
	Registry.Register(FUEAgentToolSchema(TEXT("bp_add_function"), TEXT("Blueprint"), TEXT("Add a function graph to a Blueprint"), { FUEAgentToolParam(TEXT("bp_path"), TEXT("string"), TEXT("Blueprint asset path"), true), FUEAgentToolParam(TEXT("function_name"), TEXT("string"), TEXT("New function name"), true), FUEAgentToolParam(TEXT("inputs"), TEXT("array"), TEXT("Optional input pin definitions"), false, FString(), TEXT("object")), FUEAgentToolParam(TEXT("outputs"), TEXT("array"), TEXT("Optional output pin definitions"), false, FString(), TEXT("object")) }), &HandleBpAddFunction);
	Registry.Register(FUEAgentToolSchema(TEXT("bp_create_node"), TEXT("Blueprint"), TEXT("Create a node inside a Blueprint graph"), { FUEAgentToolParam(TEXT("bp_path"), TEXT("string"), TEXT("Blueprint asset path"), true), FUEAgentToolParam(TEXT("graph_name"), TEXT("string"), TEXT("Target graph name"), true), FUEAgentToolParam(TEXT("node_class"), TEXT("string"), TEXT("Node class name"), true), FUEAgentToolParam(TEXT("position"), TEXT("object"), TEXT("Optional {x,y} node position")) }), &HandleBpCreateNode);
	Registry.Register(FUEAgentToolSchema(TEXT("bp_connect_pins"), TEXT("Blueprint"), TEXT("Connect two Blueprint node pins"), { FUEAgentToolParam(TEXT("bp_path"), TEXT("string"), TEXT("Blueprint asset path"), true), FUEAgentToolParam(TEXT("graph_name"), TEXT("string"), TEXT("Target graph name"), true), FUEAgentToolParam(TEXT("source_node_id"), TEXT("string"), TEXT("Source node GUID"), true), FUEAgentToolParam(TEXT("source_pin"), TEXT("string"), TEXT("Source pin name"), true), FUEAgentToolParam(TEXT("target_node_id"), TEXT("string"), TEXT("Target node GUID"), true), FUEAgentToolParam(TEXT("target_pin"), TEXT("string"), TEXT("Target pin name"), true) }), &HandleBpConnectPins);
...
UBlueprint* FUEAgentBpAtomicAPI::LoadBlueprint(const FString& BpPath)
{
	return BpPath.IsEmpty() ? nullptr : LoadObject<UBlueprint>(nullptr, *NormalizeObjectPath(BpPath));
}

UEdGraph* FUEAgentBpAtomicAPI::FindGraph(UBlueprint* Blueprint, const FString& GraphName)
{
	if (!Blueprint)
	{
		return nullptr;
...
FString FUEAgentBpAtomicAPI::CreateNode(UBlueprint* Blueprint, UEdGraph* Graph, const FString& NodeClass, FVector2D Position, const TSharedPtr<FJsonObject>& ExtraParams)
{
	if (!Blueprint || !Graph)
	{
		return FString();
...
bool FUEAgentBpAtomicAPI::ConnectPins(UBlueprint* Blueprint, UEdGraph* Graph, const FString& SourceNodeId, const FString& SourcePinName, const FString& TargetNodeId, const FString& TargetPinName)
{
	FString Error;
	return TryConnectPins(Blueprint, Graph, SourceNodeId, SourcePinName, TargetNodeId, TargetPinName, Error);
}

bool FUEAgentBpAtomicAPI::SetPinDefault(UBlueprint* Blueprint, UEdGraph* Graph, const FString& NodeId, const FString& PinName, const FString& Value)
{
	FString Error;
	return TrySetPinDefault(Blueprint, Graph, NodeId, PinName, Value, Error);
}

bool FUEAgentBpAtomicAPI::CompileBlueprint(UBlueprint* Blueprint, TArray<FString>& OutErrors, bool bSkipGarbageCollection)
{
	OutErrors.Reset();
	if (!Blueprint)
	{
...
TSharedPtr<FJsonObject> FUEAgentBpAtomicAPI::HandleBpCreate(const TSharedPtr<FJsonObject>& Params)
{
	FString Error;
	if (!FUEAgentCommonUtils::ValidateRequiredParams(Params, { TEXT("name"), TEXT("parent_class"), TEXT("save_path") }, Error)) { return FUEAgentCommonUtils::CreateErrorResponse(Error); }
	...
	if (!ParentClass) { return FUEAgentCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Parent class not found: %s"), *ParentClassName)); }
	...
	if (LoadObject<UBlueprint>(nullptr, *NormalizeObjectPath(PackagePath))) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Blueprint already exists at target path")); }
	const FScopedTransaction Transaction(NSLOCTEXT("UEAgent", "BpCreate", "UEAgent: Create Blueprint"));
	...
	if (!NewBlueprint) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Failed to create Blueprint")); }
...
TSharedPtr<FJsonObject> FUEAgentBpAtomicAPI::HandleBpAddFunction(const TSharedPtr<FJsonObject>& Params)
{
	FString Error;
	if (!FUEAgentCommonUtils::ValidateRequiredParams(Params, { TEXT("bp_path"), TEXT("function_name") }, Error)) { return FUEAgentCommonUtils::CreateErrorResponse(Error); }
	...
	if (!Blueprint) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Invalid bp_path")); }
	if (FindGraph(Blueprint, FunctionName)) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Graph already exists")); }
	const FScopedTransaction Transaction(NSLOCTEXT("UEAgent", "BpAddFunction", "UEAgent: Add Blueprint Function"));
	...
	if (Inputs && !EntryNode) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Function entry node not found")); }
	if (Outputs && Outputs->Num() > 0 && !ResultNode) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Function result node not found")); }
	if (Inputs)
	{
		for (const TSharedPtr<FJsonValue>& Value : *Inputs)
		{
			...
			if (!GetNamedTypeField(PinObject, PinName, PinTypeName) || !ResolvePinType(PinTypeName, PinType)) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Invalid function input definition")); }
	...
	if (Outputs)
	{
		for (const TSharedPtr<FJsonValue>& Value : *Outputs)
		{
			const TSharedPtr<FJsonObject> PinObject = Value.IsValid() ? Value->AsObject() : nullptr;
			FString PinName; FString PinTypeName; FEdGraphPinType PinType;
			if (!GetNamedTypeField(PinObject, PinName, PinTypeName) || !ResolvePinType(PinTypeName, PinType)) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Invalid function output definition")); }
...
TSharedPtr<FJsonObject> FUEAgentBpAtomicAPI::HandleBpCreateNode(const TSharedPtr<FJsonObject>& Params)
{
	FString Error;
	if (!FUEAgentCommonUtils::ValidateRequiredParams(Params, { TEXT("bp_path"), TEXT("graph_name"), TEXT("node_class") }, Error)) { return FUEAgentCommonUtils::CreateErrorResponse(Error); }
	...
	if (!Graph) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Graph not found")); }
	const FScopedTransaction Transaction(NSLOCTEXT("UEAgent", "BpCreateNode", "UEAgent: Create Blueprint Node"));
	const FString NodeId = CreateNode(Blueprint, Graph, NodeClass, GetPositionFromJson(Params), Params);
	return NodeId.IsEmpty() ? FUEAgentCommonUtils::CreateErrorResponse(TEXT("Failed to create node")) : MakeNodeResponse(NodeId);
}

TSharedPtr<FJsonObject> FUEAgentBpAtomicAPI::HandleBpConnectPins(const TSharedPtr<FJsonObject>& Params)
{
	FString Error;
	if (!FUEAgentCommonUtils::ValidateRequiredParams(Params, { TEXT("bp_path"), TEXT("graph_name"), TEXT("source_node_id"), TEXT("source_pin"), TEXT("target_node_id"), TEXT("target_pin") }, Error)) { return FUEAgentCommonUtils::CreateErrorResponse(Error); }
	...
	UEdGraph* Graph = FindGraph(Blueprint, GraphName);
	if (!Graph) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Graph not found")); }
	const FScopedTransaction Transaction(NSLOCTEXT("UEAgent", "BpConnectPins", "UEAgent: Connect Blueprint Pins"));
	if (!TryConnectPins(Blueprint, Graph, SourceNodeId, SourcePin, TargetNodeId, TargetPin, Error)) { return FUEAgentCommonUtils::CreateErrorResponse(Error); }
	TSharedPtr<FJsonObject> Data = MakeShared<FJsonObject>();
	Data->SetBoolField(TEXT("connected"), true);
	return FUEAgentCommonUtils::CreateSuccessResponse(Data);
}

TSharedPtr<FJsonObject> FUEAgentBpAtomicAPI::HandleBpSetPinDefault(const TSharedPtr<FJsonObject>& Params)
{
	FString Error;
	if (!FUEAgentCommonUtils::ValidateRequiredParams(Params, { TEXT("bp_path"), TEXT("graph_name"), TEXT("node_id"), TEXT("pin_name"), TEXT("value") }, Error)) { return FUEAgentCommonUtils::CreateErrorResponse(Error); }
	...
	UEdGraph* Graph = FindGraph(Blueprint, GraphName);
	if (!Graph) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Graph not found")); }
	const FScopedTransaction Transaction(NSLOCTEXT("UEAgent", "BpSetPinDefault", "UEAgent: Set Blueprint Pin Default"));
	if (!TrySetPinDefault(Blueprint, Graph, NodeId, PinName, Value, Error)) { return FUEAgentCommonUtils::CreateErrorResponse(Error); }
	TSharedPtr<FJsonObject> Data = MakeShared<FJsonObject>();
	Data->SetBoolField(TEXT("updated"), true);
	return FUEAgentCommonUtils::CreateSuccessResponse(Data);
}

TSharedPtr<FJsonObject> FUEAgentBpAtomicAPI::HandleBpAddVariable(const TSharedPtr<FJsonObject>& Params)
{
	FString Error;
	if (!FUEAgentCommonUtils::ValidateRequiredParams(Params, { TEXT("bp_path"), TEXT("var_name"), TEXT("var_type") }, Error)) { return FUEAgentCommonUtils::CreateErrorResponse(Error); }
	...
	if (!ResolvePinType(VarTypeName, PinType)) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Unsupported variable type")); }
	const FScopedTransaction Transaction(NSLOCTEXT("UEAgent", "BpAddVariable", "UEAgent: Add Blueprint Variable"));
	if (!FBlueprintEditorUtils::AddMemberVariable(Blueprint, FName(*VarName), PinType, DefaultValue)) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Failed to add variable")); }
...
TSharedPtr<FJsonObject> FUEAgentBpAtomicAPI::HandleBpAddComponent(const TSharedPtr<FJsonObject>& Params)
{
	FString Error;
	if (!FUEAgentCommonUtils::ValidateRequiredParams(Params, { TEXT("bp_path"), TEXT("component_class"), TEXT("component_name") }, Error)) { return FUEAgentCommonUtils::CreateErrorResponse(Error); }
	...
	UBlueprint* Blueprint = LoadBlueprint(BpPath);
	if (!Blueprint) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Invalid bp_path")); }
	if (!Blueprint->SimpleConstructionScript) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Blueprint has no SimpleConstructionScript")); }
	UClass* ComponentClass = ResolveClassByName(ComponentClassName, UActorComponent::StaticClass(), TEXT('U'));
	if (!ComponentClass) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Component class not found")); }
	const FScopedTransaction Transaction(NSLOCTEXT("UEAgent", "BpAddComponent", "UEAgent: Add Blueprint Component"));
	USCS_Node* SCSNode = Blueprint->SimpleConstructionScript->CreateNode(ComponentClass, FName(*ComponentName));
	if (!SCSNode) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Failed to create component node")); }
...
TSharedPtr<FJsonObject> FUEAgentBpAtomicAPI::HandleBpCompile(const TSharedPtr<FJsonObject>& Params)
{
	FString Error;
	if (!FUEAgentCommonUtils::ValidateRequiredParams(Params, { TEXT("bp_path") }, Error)) { return FUEAgentCommonUtils::CreateErrorResponse(Error); }
	FString BpPath;
	Params->TryGetStringField(TEXT("bp_path"), BpPath);
	UBlueprint* Blueprint = LoadBlueprint(BpPath);
	if (!Blueprint) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Invalid bp_path")); }
	const FScopedTransaction Transaction(NSLOCTEXT("UEAgent", "BpCompile", "UEAgent: Compile Blueprint"));
