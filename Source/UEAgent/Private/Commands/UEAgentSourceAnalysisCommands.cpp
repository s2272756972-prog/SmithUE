...
void FUEAgentSourceAnalysisCommands::RegisterTools(FUEAgentToolRegistry& Registry)
{
	Registry.Register(
		FUEAgentToolSchema(TEXT("analyze_module"), TEXT("Analysis"),
			...
			{
				FUEAgentToolParam(TEXT("path"), TEXT("string"), TEXT("Absolute or project-relative path to source directory (e.g. Plugins/UEAgent/Source/UEAgent)"), true),
				FUEAgentToolParam(TEXT("depth"), TEXT("string"), TEXT("Analysis depth: 'module' (file-level overview) or 'class' (full class relationships). Default: class")),
				FUEAgentToolParam(TEXT("max_files"), TEXT("number"), TEXT("Max files to analyze (default: 200, prevents timeout on huge directories)")),
			}),
		&HandleAnalyzeModule);

	Registry.Register(
		FUEAgentToolSchema(TEXT("analyze_dependencies"), TEXT("Analysis"),
			TEXT("Analyze Build.cs module dependencies and return a nomnoml dependency graph."),
			{
				FUEAgentToolParam(TEXT("path"), TEXT("string"), TEXT("Path to directory containing .Build.cs files (or single .Build.cs path). Searches recursively."), true),
			}),
		&HandleAnalyzeDependencies);

	Registry.Register(
		FUEAgentToolSchema(TEXT("analyze_blueprints"), TEXT("Analysis"),
			TEXT("Analyze Blueprint assets in a Content Browser path and return a nomnoml inheritance + component composition diagram."),
			{
				FUEAgentToolParam(TEXT("content_path"), TEXT("string"), TEXT("Content Browser path (e.g. /Game/Blueprints)"), true),
				FUEAgentToolParam(TEXT("recursive"), TEXT("boolean"), TEXT("Search subdirectories (default: true)")),
			}),
...
TSharedPtr<FJsonObject> FUEAgentSourceAnalysisCommands::HandleAnalyzeModule(const TSharedPtr<FJsonObject>& Params)
{
	FString Path = Params->GetStringField(TEXT("path"));
	if (Path.IsEmpty()) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("path is required")); }
	...
	{
		return FUEAgentCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Directory not found: %s"), *AbsPath));
	}
	...
	if (AllFiles.Num() == 0)
	{
		return FUEAgentCommonUtils::CreateErrorResponse(TEXT("No .h/.cpp files found in the specified path"));
	}
...
TSharedPtr<FJsonObject> FUEAgentSourceAnalysisCommands::HandleAnalyzeDependencies(const TSharedPtr<FJsonObject>& Params)
{
	FString Path = Params->GetStringField(TEXT("path"));
	if (Path.IsEmpty()) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("path is required")); }
	...
	if (BuildFiles.Num() == 0)
	{
		return FUEAgentCommonUtils::CreateErrorResponse(TEXT("No .Build.cs files found in the specified path"));
	}
...
TSharedPtr<FJsonObject> FUEAgentSourceAnalysisCommands::HandleAnalyzeBlueprints(const TSharedPtr<FJsonObject>& Params)
{
	FString ContentPath = Params->GetStringField(TEXT("content_path"));
	if (ContentPath.IsEmpty()) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("content_path is required")); }
	...
	{
		return FUEAgentCommonUtils::CreateErrorResponse(FString::Printf(TEXT("No Blueprint assets found in: %s"), *ContentPath));
	}
