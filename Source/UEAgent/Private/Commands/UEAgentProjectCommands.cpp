...
void FUEAgentProjectCommands::RegisterTools(FUEAgentToolRegistry& Registry)
{
    // get_project_info
    Registry.Register(
        FUEAgentToolSchema(TEXT("get_project_info"), TEXT("Project"),
    ...
    // list_plugins
    Registry.Register(
        FUEAgentToolSchema(TEXT("list_plugins"), TEXT("Project"),
            TEXT("Lists all discovered plugins with name, version, enabled status, description, and category."),
            {
                FUEAgentToolParam(TEXT("enabled_only"), TEXT("boolean"),
    ...
    Registry.Register(
        FUEAgentToolSchema(TEXT("get_project_settings"), TEXT("Project"),
            TEXT("Reads a config value from GConfig by section and key."),
            {
                FUEAgentToolParam(TEXT("section"), TEXT("string"),
                    TEXT("Config section, e.g. /Script/Engine.RendererSettings"), true),
                FUEAgentToolParam(TEXT("key"), TEXT("string"),
    ...
    // create_folder
    Registry.Register(
        FUEAgentToolSchema(TEXT("create_folder"), TEXT("Project"),
            TEXT("Creates a content browser folder (e.g. /Game/MyFolder/SubFolder)."),
            {
                FUEAgentToolParam(TEXT("folder_path"), TEXT("string"),
...
TSharedPtr<FJsonObject> FUEAgentProjectCommands::HandleGetProjectInfo(const TSharedPtr<FJsonObject>& Params)
{
...
TSharedPtr<FJsonObject> FUEAgentProjectCommands::HandleListPlugins(const TSharedPtr<FJsonObject>& Params)
{
...
TSharedPtr<FJsonObject> FUEAgentProjectCommands::HandleGetProjectSettings(const TSharedPtr<FJsonObject>& Params)
{
    if (!Params.IsValid())
    {
        return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Missing parameters"));
    }

    FString Section;
    if (!Params->TryGetStringField(TEXT("section"), Section) || Section.IsEmpty())
    {
        return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Missing required parameter: 'section'"));
    }
    ...
    else
    {
        ...
        UE_LOG(LogUEAgent, Warning, TEXT("get_project_settings: key '%s' not found in section '%s'"), *Key, *Section);
    }
...
TSharedPtr<FJsonObject> FUEAgentProjectCommands::HandleCreateFolder(const TSharedPtr<FJsonObject>& Params)
{
    if (!Params.IsValid())
    {
        return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Missing parameters"));
    ...
    {
        return FUEAgentCommonUtils::CreateErrorResponse(TEXT("Missing required parameter: 'folder_path'"));
    }
...
TSharedPtr<FJsonObject> FUEAgentProjectCommands::HandleGetSourceFiles(const TSharedPtr<FJsonObject>& Params)
{
    // Determine base path
    FString BasePath;
