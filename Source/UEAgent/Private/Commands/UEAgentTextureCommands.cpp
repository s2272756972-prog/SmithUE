...
namespace
{
	...
	void OnApiResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful, const FString& TaskId, EApiFormat Format)
	{
		...
		UE_LOG(LogUEAgent, Log, TEXT("generate_texture: Task %s completed → %s"), *TaskId, *Task.AssetPath);
	}
}
...
void FUEAgentTextureCommands::RegisterTools(FUEAgentToolRegistry& Registry)
{
	Registry.Register(
		FUEAgentToolSchema(TEXT("generate_texture"), TEXT("Asset"),
			TEXT("Generate a texture from a text prompt using an external AI image generation API. Returns a task_id for polling."),
			{
				FUEAgentToolParam(TEXT("prompt"), TEXT("string"), TEXT("Text prompt describing the desired texture"), true),
				FUEAgentToolParam(TEXT("endpoint"), TEXT("string"), TEXT("AI API endpoint URL (auto-detects DALL-E/Imagen/OpenAI/Google format)"), true),
				FUEAgentToolParam(TEXT("api_key"), TEXT("string"), TEXT("API authentication key (Bearer token or Google API key)"), true),
				FUEAgentToolParam(TEXT("save_path"), TEXT("string"), TEXT("Content Browser path for the asset (default: /Game/GeneratedTextures)")),
				FUEAgentToolParam(TEXT("asset_name"), TEXT("string"), TEXT("Custom asset name (default: auto-generated timestamp)")),
				FUEAgentToolParam(TEXT("model"), TEXT("string"), TEXT("Model name to pass to the API (e.g. dall-e-3, imagen-4.0)")),
				FUEAgentToolParam(TEXT("aspect_ratio"), TEXT("string"), TEXT("Image size/ratio (e.g. 1024x1024, 16:9)")),
				FUEAgentToolParam(TEXT("create_material"), TEXT("boolean"), TEXT("Auto-create a material with this texture (default: false)")),
			}),
		&HandleGenerateTexture);

	Registry.Register(
		FUEAgentToolSchema(TEXT("check_generation_task"), TEXT("Asset"),
			TEXT("Check the status of an asynchronous texture generation task"),
			{
				FUEAgentToolParam(TEXT("task_id"), TEXT("string"), TEXT("Task ID returned by generate_texture"), true),
			}),
...
TSharedPtr<FJsonObject> FUEAgentTextureCommands::HandleGenerateTexture(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (Prompt.IsEmpty()) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("prompt is required")); }
	if (Endpoint.IsEmpty()) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("endpoint is required")); }
	if (ApiKey.IsEmpty()) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("api_key is required")); }
	...
	UE_LOG(LogUEAgent, Log, TEXT("generate_texture: Started task %s (endpoint: %s)"), *TaskId, *Endpoint.Left(80));
...
TSharedPtr<FJsonObject> FUEAgentTextureCommands::HandleCheckGenerationTask(const TSharedPtr<FJsonObject>& Params)
{
	FString TaskId = Params->GetStringField(TEXT("task_id"));
	if (TaskId.IsEmpty()) { return FUEAgentCommonUtils::CreateErrorResponse(TEXT("task_id is required")); }

	TSharedPtr<FTextureTask>* TaskPtr = TaskStore.Find(TaskId);
	if (!TaskPtr || !TaskPtr->IsValid()) { return FUEAgentCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Task not found: %s"), *TaskId)); }
