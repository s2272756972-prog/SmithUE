...
void FSmithUEObservationCommands::RegisterTools(FSmithUEToolRegistry& Registry)
{
	// list_panels
	Registry.Register(
		FSmithUEToolSchema(TEXT("list_panels"), TEXT("Observation"),
	...
	// open_panel
	Registry.Register(
		FSmithUEToolSchema(TEXT("open_panel"), TEXT("Observation"),
			TEXT("Opens (or focuses) a named editor panel tab."),
			{
				FSmithUEToolParam(TEXT("panel_name"), TEXT("string"),
	...
	// close_panel
	Registry.Register(
		FSmithUEToolSchema(TEXT("close_panel"), TEXT("Observation"),
	...
	// get_editor_state
	Registry.Register(
		FSmithUEToolSchema(TEXT("get_editor_state"), TEXT("Observation"),
	...
	// get_selected_actors
	Registry.Register(
		FSmithUEToolSchema(TEXT("get_selected_actors"), TEXT("Observation"),
	...
	// get_world_outline
	Registry.Register(
		FSmithUEToolSchema(TEXT("get_world_outline"), TEXT("Observation"),
			TEXT("Returns all actors in the level with parent-child hierarchy and folder info."),
			{
				FSmithUEToolParam(TEXT("filter"), TEXT("string"), TEXT("Optional substring filter on actor label")),
				FSmithUEToolParam(TEXT("max_depth"), TEXT("number"), TEXT("Ignored in v1 (flat list returned)"))
			}),
...
TSharedPtr<FJsonObject> FSmithUEObservationCommands::HandleListPanels(const TSharedPtr<FJsonObject>& Params)
{
	TArray<FString> KnownPanels = SmithUEEditorState::GetKnownPanelNames();
...
TSharedPtr<FJsonObject> FSmithUEObservationCommands::HandleOpenPanel(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FSmithUECommonUtils::CreateErrorResponse(TEXT("Missing required parameter: panel_name"));
	}
...
TSharedPtr<FJsonObject> FSmithUEObservationCommands::HandleClosePanel(const TSharedPtr<FJsonObject>& Params)
{
	if (SmithUEEditorState::IsInPIE())
	{
		return SmithUEEditorState::CreatePIEErrorResponse();
...
TSharedPtr<FJsonObject> FSmithUEObservationCommands::HandleGetEditorState(const TSharedPtr<FJsonObject>& Params)
{
	if (!GEditor)
	{
	...
	bool bIsPIE = SmithUEEditorState::IsInPIE();
	bool bIsSimulating = GEditor->bIsSimulatingInEditor;
	bool bModalOpen = SmithUEEditorState::IsModalDialogOpen();
	bool bViewportAvailable = SmithUEEditorState::GetActiveViewportClient() != nullptr;
	...
	if (FEditorViewportClient* C = SmithUEEditorState::GetActiveViewportClient()) {
...
TSharedPtr<FJsonObject> FSmithUEObservationCommands::HandleGetLevelInfo(const TSharedPtr<FJsonObject>& Params)
{
	if (!GEditor)
	{
		return FSmithUECommonUtils::CreateErrorResponse(TEXT("GEditor is not available"));
...
TSharedPtr<FJsonObject> FSmithUEObservationCommands::HandleGetActorProperty(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorLabel;
	...
	else if (Prop->IsA<FStructProperty>())
	{
		...
		if (!bOK)
			return FSmithUECommonUtils::CreateErrorResponse(FString::Printf(TEXT("Struct '%s' not supported for '%s'"), *SN.ToString(), *PropertyName));
		FString ExportedStr;
...
TSharedPtr<FJsonObject> FSmithUEObservationCommands::HandleGetSelectedActors(const TSharedPtr<FJsonObject>& Params)
{
...
TSharedPtr<FJsonObject> FSmithUEObservationCommands::HandleGetWorldOutline(const TSharedPtr<FJsonObject>& Params)
{
	if (!GEditor) return FSmithUECommonUtils::CreateErrorResponse(TEXT("GEditor not available"));
	UWorld* World = GEditor->GetEditorWorldContext().World();
	if (!World) return FSmithUECommonUtils::CreateErrorResponse(TEXT("No editor world available"));
