...
void FSmithUEUMGCommands::RegisterTools(FSmithUEToolRegistry& Registry)
{
    ...
    Registry.Register(
        FSmithUEToolSchema(
            TEXT("add_widget"),
            TEXT("UMG"),
            TEXT("Add a widget to an existing Widget Blueprint's widget tree"),
            {
...
TSharedPtr<FJsonObject> FSmithUEUMGCommands::HandleCreateWidgetBlueprint(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (RootClass && WidgetBP->WidgetTree)
    {
        ...
        if (RootWidget)
        {
            ...
            // Register in WidgetVariableNameToGuidMap (UE 5.7, unconditional)
            FGuid NewGuid = FGuid::NewGuid();
            WidgetBP->WidgetVariableNameToGuidMap.Add(RootWidget->GetFName(), NewGuid);
        }
    }
