...
// Helper macros and utilities for SmithUE tests
namespace SmithUETestUtils
{
    ...
    {
        return FSmithUEToolRegistry::Get().DispatchCommand(Command, Params);
    }

    // Dispatch with no params
    inline TSharedPtr<FJsonObject> DispatchEmpty(const FString& Command)
    {
        return FSmithUEToolRegistry::Get().DispatchCommand(Command, EmptyParams());
    }
