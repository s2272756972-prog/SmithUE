...
## v1.15.1-UE5.1（UE5.1 兼容分支，2026-08-30）
...
- **插件描述符**：版本标记为 `1.15.1-UE5.1`，安装文档、部署说明和示例分支同步改为 `UE5.1`；保留 UE5.1 可识别的描述符字段。
...
- 将打包产物装入最小 UE5.1 项目并用 `-NullRHI` 启动真实编辑器；`smithue-cli status --wait` 返回 `ready:true`、`version:1.15.1-UE5.1` 和实际 `engine_version:5.1.1-23901901+++UE5+Release-5.1`，只读 `ping` 返回 `pong`。

### 已知边界

- 当前验证覆盖 **UE5.1 / Win64 的插件构建、打包、最小项目 `-NullRHI` 编辑器启动与只读 CLI 连通性**；尚未在具体业务项目中完成 AnimGraph 写入、资产修改和 PIE 全链路验收。
