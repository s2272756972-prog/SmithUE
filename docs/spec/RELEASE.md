...
>
> 占位符同 [AGENTS.md](../../AGENTS.md)：`{EngineRoot}` = UE 5.1 安装根、`{ProjectRoot}` = 宿主工程根、`{Project}` = 宿主工程名。**勿把绝对路径写死。**

## 0. 前提

- 仓库 `s2272756972-prog/SmithUE`，兼容分支 `UE5.1`。
- 与 `smithue-cli` 版本号**完全独立**，禁止比较 / 对齐。
- Release tag 约定 **`vX.Y.Z-UE5.1`**；资产命名 **`SmithUE-vX.Y.Z-UE5.1-Win64.zip`**。
...
- `CHANGELOG.md`：将当前 UE5.1 兼容条目更新为实际版本与日期。
...
Compress-Archive -Path "{ProjectRoot}\Saved\SmithUE" `
  -DestinationPath "{ProjectRoot}\Saved\SmithUE-vX.Y.Z-UE5.1-Win64.zip" -CompressionLevel Optimal
```
zip 根应为 `SmithUE/`，用户可直接解压进宿主 `Plugins/`。

### 3.8 GitHub Release（notes 用 Write 写，CJK 安全）
```powershell
gh release create vX.Y.Z-UE5.1 `
  --repo s2272756972-prog/SmithUE --target UE5.1 `
  --title "SmithUE vX.Y.Z (UE 5.1)" `
  --notes-file <notes.md> --latest `
  "{ProjectRoot}\Saved\SmithUE-vX.Y.Z-UE5.1-Win64.zip"
gh release view vX.Y.Z-UE5.1 --repo s2272756972-prog/SmithUE   # 核验 Latest + 资产
```
