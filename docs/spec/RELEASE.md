...
>
> 占位符同 [AGENTS.md](../../AGENTS.md)：`{EngineRoot}` = UE 5.8 安装根、`{ProjectRoot}` = 宿主工程根、`{Project}` = 宿主工程名（当前环境 `{Project}` = AIScript）。**勿把绝对路径写死。**

## 0. 前提

- 仓库 `123dx-svg/SmithUE`，**默认分支 `UE5.8`（不是 main）**。
- 与 `smithue-cli` 版本号**完全独立**，禁止比较 / 对齐。
- Release tag 约定 **`vX.Y.Z-UE5.8`**；资产命名 **`SmithUE-vX.Y.Z-UE5.8-Win64.zip`**。
- 编译目标是 **`{Project}Editor`**（宿主 target，当前即 `MassDemoEditor`），不是 "SmithUE"。
...
- `CHANGELOG.md`：`## 未发布（当前 UE5.8 分支）` → `## vX.Y.Z（UE5.8，YYYY-MM-DD）`。
...
Compress-Archive -Path "{ProjectRoot}\Saved\SmithUE" `
  -DestinationPath "{ProjectRoot}\Saved\SmithUE-vX.Y.Z-UE5.8-Win64.zip" -CompressionLevel Optimal
```
zip 根应为 `SmithUE/`，用户可直接解压进宿主 `Plugins/`。

### 3.8 GitHub Release（notes 用 Write 写，CJK 安全）
```powershell
gh release create vX.Y.Z-UE5.8 `
  --repo 123dx-svg/SmithUE --target UE5.8 `
  --title "SmithUE vX.Y.Z (UE 5.8)" `
  --notes-file <notes.md> --latest `
  "{ProjectRoot}\Saved\SmithUE-vX.Y.Z-UE5.8-Win64.zip"
gh release view vX.Y.Z-UE5.8 --repo 123dx-svg/SmithUE   # 核验 Latest + 资产
```
